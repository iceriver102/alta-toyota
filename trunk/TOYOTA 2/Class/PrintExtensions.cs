        ...
        public static bool PrintImage(this FileInfo file, int id)
        {
            if (file.Exists)
            {
                ...
                if (extensions == ".png" || extensions == ".jpg" || extensions == ".bmp")
                {
                    ...
                    dc.DrawImage(bi, new Rect { Width = capabilities.PageImageableArea.ExtentWidth, Height = capabilities.PageImageableArea.ExtentHeight });
                    dc.Close();
                    dialog.PrintVisual(vis, PrinterSettings.InstalledPrinters[id]);
                    */
                    return true;
                }
                else
